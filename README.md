...
```
python3 pytorch_rl/main.py --num-processes=32 --expName=BabyAI_FullSignal --useMissionAdvice=1
```
...
```
obsFull={image:tensor[3,H,W], mission:'go ahead Baby!', bestActionID:[1,2]}
```
bestActionID is the set of the optimal actions computed by the agent, we take care of the fact where multiple optimal actions can be realised.
The best actions are not given to the baby, however they are used to measure how the baby follows the teacher instructions.

You can decide the frequency to which the agent will have access to the mission string witth the argument ```--useMissionAdvice```. Set this value to 1 will allow the agent to listen to the teacher every 1 step.


Notice that in order to communicate the advice strings generated by the techer between the different threads, we converted them as list of ASCII codes, of maximum shape 200 (arbitrary value to guarantee to have enough memory to store all the messages).

We also implemented nice visualizations using visdom that allow you to track the evolution of your baby, define your server and your port in the ```arguments.py``` file. 

![plot of the visdom visualization tool](https://github.com/maximecb/baby-ai-game/blob/simon2/visdom.png)
