    ...
    for x, y, z in tab:
        ...
        if x == "N_death" or x == "Reward_mean":
            ...
            if z:
                area_top = []
                area_bot = []
                for k in range(0, len(mean_array_mon[i + 1])):
                    area_top.append(mean_array_mon[i][k] + mean_array_mon[i + 1][k])
                    area_bot.append(mean_array_mon[i][k] - mean_array_mon[i + 1][k])
                plt.fill_between(mean_array_mon[0], area_bot, area_top, color="red", alpha=0.4)

                area_top = []
                area_bot = []
                for k in range(0, len(mean_array_nomon[i + 1])):
                    area_top.append(mean_array_nomon[i][k] + mean_array_nomon[i + 1][k])
        ...
        if y =="N_goal_reached":
            ...
            j += 1
            color = 'r'
            if len(mean_array_mon[i + 1]) > 0:
                plt.plot(mean_array_mon[0], mean_array_mon[i + 1], color, linewidth=1, label=y + "_monitor")
            color = 'b'
